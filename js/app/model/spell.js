define(['can'], function(can) {
    ...
    var spec_specific = {
        ...
        fheal_pally: function(delta) {
            return(this.fbase(delta)*
                (1+(-1+ 2*(this.spec.critmeta? 1.03 : 1))*this.spec.fcritp(delta))*
                (1+ (this.spec.bol ? 0.5 : 0) + this.spec.fmastp(delta)));
        },
        ...
        fheal_disc: function(delta) {
            var da = 0.5;
            return(this.fbase(delta)*
                (1+(-1+ 2*(this.spec.critmeta? 1.03 : 1) *
    ...
    return(this.fbase(delta)*(1+this.spec.fmastp(delta))*(1+this.spec.fcritp(delta))*(1+ 0.3*this.spec.fcritp(delta)));
        ...
        fheal_sth_holy: function(delta) {
            return(this.fbase(delta)*
                (1+(-1+ 2*(this.spec.critmeta? 1.03 : 0))*
                    ((this.spec.serenity ? 0.25 : 0) + this.spec.fcritp(delta)))*
                    (1+this.spec.fmastp(delta)));
        },
        fheal_living_seed: function(delta) {
            return(this.fbase(delta)*(1+this.spec.fmastp(delta))*
                (1+(-1+ 2*(this.spec.critmeta? 1.03 : 1)*1.3)*this.spec.fcritp(delta)));
        },
    ...
    spls.find('CascadeDisc').attr({
        fmana: function(delta) {return(this.fmana_instant_priest(delta));},
        ftargets: function(delta) {return(this.spec.cascade_targets_disc * (0.4+Math.min(this.spec.cascade_range_disc, 30)*0.02))
        }
    });
    spls.find('PWSDisc').attr({
        fmana: function(delta) {return(this.fmana_instant_priest(delta));},
        fbase: function(delta) {return(this.fdirect(delta))*(this.spec.attr('archangel') ? 1.25 : 1);},
        fheal: function(delta) {return(this.fbase(delta) * (1+this.spec.fmastp(delta)));},
    });
    spls.find('PoMDisc').attr({
        fmana: function(delta) {return(this.fmana_instant_priest(delta));},
        fdirect: function(delta) { return((this.B+this.c*this.spec.fsp(delta)) * (this.spec.attr('glyph_pom_disc') ? 5.6 : 6) ); },
    });
    spls.find('PoHDisc').attr({
        ...
        ftargets: function(delta) {return(1*this.spec.poh_targets_disc);},
    });
    ...
    spls.find('PoHSSDisc').attr({
        fheal: function(delta) { return(this.fbase(delta) * (1+this.spec.fcritp(delta)) * (1+this.spec.fmastp(delta)) * 1.3 );},
        ftargets: function(delta) {return(1*this.spec.poh_targets_disc);},
    });
    spls.find('HolyFireDisc').attr({
        ...
        fheal: function(delta) {return(this.fheal_disc_atonement(delta));},
        fmana: function(delta) { return(this.base_mana * (1-this.spec.evang_stacks*0.06))},
    });
    ...
    spls.find('PenanceDisc').attr({
        ...
        fheal: function(delta) {return(this.fheal_disc_atonement(delta));}, // This actually assumes penance is used as atonement
        fmana: function(delta) { return(this.base_mana * (1-this.spec.evang_stacks*0.06))},
});
    ...
    $.each(['HealHoly', 'FhealHoly', 'GhealHoly', 'BhealHoly'], function(i, spname) {
        ...
        fbase: function(delta) {return(this.fbase_holy_sth(delta));},
        fheal: function(delta) {return(this.fheal_sth_holy(delta));}
        });
    });
    ...
    $.each(['PoHHoly', 'DHHoly', 'CoHHoly', 'PoMHoly', 'HWSanctuaryHoly'], function(i, spname) {
        spls.find(spname).attr({
            fbase: function(delta) {return(this.fbase_holy_aoe(delta));},
        });
    });
    ...
    spls.find('CascadeHoly').attr({
        ftargets: function(delta) {return(this.spec.cascade_targets_holy * (0.4+Math.min(this.spec.cascade_range_holy, 30)*0.02))}
    });
    spls.find('PoHHoly').attr({
        ftargets: function(delta) {return(1*this.spec.poh_targets_holy);},
    });
    spls.find('PoMHoly').attr({
        fmana: function(delta) {return(this.fmana_instant_priest(delta));},
        fdirect: function(delta) { return((this.B+this.c*this.spec.fsp(delta)) * (this.spec.attr('glyph_pom_holy') ? 5.6 : 6) ); }
    });
    spls.find('RenewHoly').attr({
        ...
        fbase: function(delta) { return(this.fhot(delta) * 1.15 * 1.15 * (this.spec.sanctuary ? 1.25 : 1));},
        fct: function(delta) { return 1;},
    });
    spls.find('PWSHoly').attr({
        fmana: function(delta) {return(this.fmana_instant_priest(delta));},
        fheal: function(delta) {return(this.fbase(delta));},
    });
    ...
    spls.find('Nourish').attr({
        fct: function(delta) { return((this.base_ct - (this.spec.attr('glyph_rejuv') ? 0.5 : 0))/(1+this.spec.fhastep(delta))); },
        fbase: function(delta) { return(this.fdirect(delta) * 1.2);},
        fheal: function(delta) {return(this.fheal_living_seed(delta));},
    });
    spls.find('HealingTouch').attr({ fheal: function(delta) {return(this.fheal_living_seed(delta));} });
    spls.find('Regrowth').attr({
        fbase: function(delta) {return(this.fdirect(delta) + (this.spec.attr('glyph_regrowth') ? 0 : this.fhot(delta)));},
        fheal: function(delta) { 
            var crit = (this.spec.attr('glyph_regrowth') ? 1 : (Math.min(this.spec.fcritp(delta) + 0.6, 1)));
            return((1+this.spec.fmastp(delta))*(this.fbase(delta)*
            (1+(-1+2*crit*(this.spec.critmeta ? 1.03 : 1))) +
                                        this.fdirect(delta) *
                                        (this.spec.critmeta ? 1.03 : 1) * 0.6 * crit)
            ); 
        },
    });
    ...
    spls.find('Lifebloom').attr({
        ...
        fbase: function(delta) {return(this.fdirect(delta) + this.fhot(delta));}
    });
    ...
    spls.find('Swiftmend').attr({
        ...
        fheal: function(delta) {return( (1+this.spec.fmastp(delta)) *
            (
                this.fbase(delta) * (1+(-1+2*(this.spec.critmeta? 1.03 : 1))*
                                            this.spec.fcritp(delta))  +
                (this.Btick + this.ctick*this.spec.fsp(delta)) * 0.6 *
                  (this.spec.critmeta? 1.03 : 1) * this.spec.fcritp(delta)
            )
        )}
    });
    spls.find('Mushrooms').attr({
        fheal: function(delta) {
            return(this.fbase(delta)*(1+(-1+2*(this.spec.critmeta? 1.03 : 1))*
                                            this.spec.fcritp(delta)));
        }
    });
    ...
    spls.find('ForceOfNature').attr({
        fnticks: function(delta) {return(this.fnticks_shaman_aoe(delta));},
        fhpm: function(delta) {return(this.fhpm_nomana(delta));}
    });
    ...
    spls.find('HolyShock').attr({
        fheal: function(delta) { 
            ...
            return((1+ (this.spec.bol ? 0.5 : 0) + this.spec.fmastp(delta))*this.fbase(delta)*
            (1+(-1+2*(this.spec.critmeta? 1.03 : 1))*crit) *
                    (this.spec.attr('daybreak') ? 2 : 1)); 
        }
    });
    spls.find('WoG').attr({
        fct: function(delta) {return(this.fct_hp_pally(delta));},
        fmana: function(delta) {return(this.fmana_hp_pally(delta));},
        fhpm: function(delta) {return(this.fhpm_hp_pally_nomana(delta));},
    ...
    spls.find('LoD').attr({
        ...
        fhpm: function(delta) {return(this.fhpm_hp_pally_nomana(delta));},
        fbase: function(delta) {return(this.fbase_pally_hp(delta));},
        ftargets: function(delta) {return(this.targets - (this.spec.attr('glyph_lod') ? 2 : 0))},
        fdirect: function(delta) { return((this.B+this.c*this.spec.fsp(delta)) *
             this.ftargets(delta) *
    ...
    spls.find('LightsHammer').attr({
        ...
        fheal: function(delta) {
            return(this.fbase(delta)*
                (1+(-1+2*(this.spec.critmeta? 1.03 : 1))*this.spec.fcritp(delta)) *
                (1+ (this.spec.bol ? 0.15 : 0)));
        }
    });
    spls.find('Execution').attr({ fhpm: function(delta) {return(this.fhpm_nomana(delta));} });
    spls.find('EternalFlame').attr({
        fct: function(delta) {return(this.fct_hp_pally(delta));},
        fmana: function(delta) {return(this.fmana_hp_pally(delta));},
        fhpm: function(delta) {return(this.fhpm_hp_pally_nomana(delta));},
        fbase: function(delta) {return((this.fhot(delta) + this.fdirect(delta)) *
            1.05 * 1.25 * (this.spec.one_hp ? 1 : 3)); }   
        });
    spls.find('SacredShield').attr({
        fhpm: function(delta) {return(this.fhpm_nomana(delta));},
        fbase: function(delta) {return(this.fhot(delta));}
    });
    ...
    // SHAMAN Spells setup
    spls.find('HW').attr({
        fmana: function(delta) {return(this.fmana_shaman_resurgence(delta));},
    ...
    spls.find('ChainHeal').attr({
        fmana: function(delta) {return(this.fmana_shaman_resurgence(delta));},
        ftargets: function(delta) {return(this.targets + (this.spec.earth_shield_buff ? 0.2 : 0))},
        fbase: function(delta) { 
    ...
    spls.find('HST').attr({ fnticks: function(delta) {return(this.fnticks_shaman_aoe(delta));} });
    spls.find('HTT').attr({ fnticks: function(delta) {return(this.fnticks_shaman_aoe(delta));} });
    ...
    //
    // MONK Spells setup
    spls.find('EnvelopingMist').attr({ fhpm: function(delta) {return(this.fhpm_nomana(delta));} });
    spls.find('SurgingMist').attr({
        fmana: function(delta) { return(this.base_mana * (this.spec.attr('t14_2p_monk') ? 0.9 : 1))}
    });
    spls.find('Uplift').attr({ fhpm: function(delta) {return(this.fhpm_nomana(delta));} });
